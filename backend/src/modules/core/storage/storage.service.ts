...
export class StorageService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (storageType === 's3' || storageType === 'minio') {
      this.s3Client = new S3Client({
        ...
        endpoint: storageType === 'minio' ? this.configService.get<string>('s3.url') :`http://localhost:9000`, // MinIO endpoint
      });
    }
  }
  ...
  private async saveToLocal(file: Express.Multer.File): Promise<string> {
    const uploadPath = this.configService.get<string>('file.uploadPath');
